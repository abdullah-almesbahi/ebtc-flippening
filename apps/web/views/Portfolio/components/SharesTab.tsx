...
import ShareInfoRow from '@/views/Shares/components/ShareInfoRow'
import SharesPlaceholder from '@/views/Shares/components/SharesPlacehoder'

export default function SharesTab({ data }: { data?: PortfolioDataQuery['shares'] }): JSX.Element {
  ...
  return (
    <div className="side-bar details list-items col-lg-6 m-auto">
      <div className="widget widget-recent-post mg-bt-43">
        {data ? (
          <ul>{data?.map((item, index) => <ShareInfoRow index={index} item={item} key={item.id} />)}</ul>
        ) : (
          <SharesPlaceholder />
        )}
      </div>
